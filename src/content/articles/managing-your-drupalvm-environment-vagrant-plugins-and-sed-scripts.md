...
description: Tips for taming multiple DrupalVM environments, from Vagrant
  plugins to SED scripts that keep config.yml sane.
pubDate: 2018-01-02
...
My other favorite plugin is vagrant-autonetwork, which manages local IP addresses if you have multiple machines running on your computer. Instead of manually adding an IP address (which is sometimes necessary for external tunneling), you can just put in 0.0.0.0 and let Auto Network assign one for you!
