...
**Undefined index: distribution\_name in drupal\_install\_profile\_distribution\_name()**
...
**if** ( ! array\_key\_exists(‘distribution\_name’, $info)) $info\[‘distribution\_name’\] = ‘Drupal’;
...
mysqldump -u root -p\[root\_password\] \[database\_name\] | gzip > dumpfilename.sql
...
$conf\[‘stage\_file\_proxy\_origin’\] = ‘http://example.com’;
...
drush pm-list –status=enabled –type=module –pipe > enabled\_modules\_now.txt
