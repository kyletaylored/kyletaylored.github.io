...
| YouTube embed (not a comp — added for a real article that needed one) | [`YouTubeEmbed.astro`](./YouTubeEmbed.astro) — `id` (the video ID, not the full URL), `title` (required, used as the iframe's accessible name and default caption), optional `caption`. `youtube-nocookie.com`, lazy-loaded. Mirrors `ImageGallery`'s figure/figcaption styling so a video reads as the same kind of media block as an image | `automatic-theme-generation-using-bootstrap-and-less.mdx` — also the reason that article is `.mdx` and not `.md` |
...
A third route, `src/pages/og/teaser/[...slug].png.ts`, generates a
_textless_ square (1200×1200) image per article/project/lab entry — this
is what `ArticleTeaserCard.astro` falls back to, not the article-share
card above. Reusing the share card (designed as a 1.9:1 image with a
large title baked in) as a small/square teaser thumbnail used to
object-cover-crop the title text off mid-word (jam.dev issue #4) — a
square, text-free Memphis composition (4 fixed shape recipes, picked by
hashing the entry's slug, same idea as the card's own gradient rotation)
has nothing that can run off, since every teaser crop ratio is
narrower-or-equal to the 1:1 source and only ever trims top/bottom.
